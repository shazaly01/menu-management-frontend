  ...
  >
    <div class="flex h-16 items-center justify-between px-5 flex-shrink-0">
      <RouterLink :to="{ name: 'Dashboard' }" class="flex items-center gap-2">
        ...
        <h1 class="text-xl font-bold text-text-primary">Menu</h1>
      </RouterLink>
      <button
...
import {
  ...
  ChevronDownIcon,
  QueueListIcon,
  QrCodeIcon, // أضفنا هذه الأيقونة لتمثيل الباركود
...
// --- بنية البيانات الجديدة لقائمة نظام المطعم والكاشير ---
const allNavLinks = shallowRef([
  ...
  // --- قسم الويتر / المطعم ---
  {
    name: 'الطلبات والمطعم',
    icon: QueueListIcon,
    permission: 'categories.view',
    children: [
      {
        name: 'قائمة الطعام (المنيو)',
        routeName: 'WaiterMenu',
        permission: 'categories.view',
      },
      {
        name: 'سلة الطلبات',
      ...
      {
        name: 'مراقبة الطلبات',
        routeName: 'RemoteOrdersList',
        permission: 'remote_orders.view',
      },
      {
        name: 'طباعة الباركود',
        routeName: 'MenuQRCodeView',
        permission: 'categories.view',
      },
    ],
  },
